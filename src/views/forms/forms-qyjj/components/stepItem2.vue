  ...
  <cardVue :collapse="false" title="企业简介">
    <van-form ref="vanFormRef" required="auto" label-width="100">
      <van-cell-group inset>
        ...
        <van-field v-model="inf.registerMoney" name="注册资本" label="注册资本" placeholder="请输入注册资本" :rules="[{ required: true, message: '请填写注册资本' }]" type="digit">
        ...
        <van-field v-model="inf.totalAssets" name="企业总资产" label="企业总资产" placeholder="请输入企业总资产" :rules="[{ required: true, message: '请填写企业总资产' }]" type="digit">
          <template #right-icon>万元</template>
        </van-field>
        <van-field v-model="inf.totalDebt" name="企业总负债" label="企业总负债" placeholder="请输入企业总负债" :rules="[{ required: true, message: '请填写企业总负债' }]" type="digit">
          <template #right-icon>万元</template>
        </van-field>
        <van-field v-model="inf.totalProceed" name="涉诉总金额" label="涉诉总金额" placeholder="请输入涉诉总金额" :rules="[{ required: true, message: '请填写涉诉总金额' }]" type="digit">
          <template #right-icon>万元</template>
        </van-field>
        <van-field v-model="inf.lastYearIncome" name="上年度营收" label="上年度营收" placeholder="请输入上年度营收" :rules="[{ required: true, message: '请填写上年度营收' }]" type="digit">
          <template #right-icon>万元</template>
        </van-field>
        <van-field v-model="inf.lastYearTax" name="上年度纳税额" label="上年度纳税额" placeholder="请输入上年度纳税额" :rules="[{ required: true, message: '请填写上年度纳税额' }]" type="digit">
          <template #right-icon>万元</template>
        </van-field>
        <van-field v-model="inf.lastYearProfit" name="上年度净利润" label="上年度净利润" placeholder="请输入上年度净利润" :rules="[{ required: true, message: '请填写上年度净利润' }]" type="digit">
          <template #right-icon>万元</template>
        </van-field>
        <van-field class="textarea-bg" maxlength="200" show-word-limit label-align="top" type="textarea" v-model="inf.businessScopes" name="主营业务描述" label="主营业务描述" placeholder="请输入主营业务描述"> </van-field>
